...
int precedence(char op1, char op2) {
    ...
    switch (op1) {
        case '+': case '-': precedence1 = 1; break;
        case '*': case '/': precedence1 = 2; break;
        default: return -1; 
    }

    switch (op2) {
        case '+': case '-': precedence2 = 1; break;
        case '*': case '/': precedence2 = 2; break;
        default: return -1; 
    }

    if (precedence1 == precedence2) return 0;
...
int parse_expression(char exp[]) {
    ...
    while (i < strlen(exp)) {
        ...
        if (isOperand(exp[i])) {
            ...
            // means the expression starts with an operator
            if(od_top == -1){
                return -1;
            }

            if(i+1 == strlen(exp) || isOperator(exp[i+1])){
                return -1;
            }
